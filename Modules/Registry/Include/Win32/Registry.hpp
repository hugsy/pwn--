...
///@file registry.hpp
///
///@brief Interface to windows::Registry, a class to easily manipulate Windows registry
///
...
constexpr usize MAX_REGSZ_KEY_SIZE        = 255;
constexpr usize MAX_REGSZ_VALUE_NAME_SIZE = 16383;
constexpr usize MAX_REGSZ_VALUE_SIZE      = 1 * 1024 * 1024;
...
///
using RegistryHandle = GenericHandle<HKEY__, ::RegCloseKey>;
...
public:
        ...
        const usize Size = sizeof(T))
    {
        ...
        else if constexpr ( std::is_same_v<T, std::vector<std::wstring>> ) // REG_MULTI_SZ
        {
            ...
            KeyValue = std::move(Utils::StringLib::Split(TempValue, L'\0'));
        }
