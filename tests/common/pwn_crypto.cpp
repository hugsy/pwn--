...
TEST_CASE("MD tests", "[" NS "]")
{
    SECTION("MD2")
    {
        ...
        CHECK(pwn::crypto::md2(data) == expected);
    }
    ...
    SECTION("MD4")
    {
        ...
        CHECK(pwn::crypto::md4(data) == expected);
    }
    ...
    SECTION("MD5")
    {
        ...
        CHECK(pwn::crypto::md5(data) == expected);
    }
}
...
TEST_CASE("SHA tests", "[" NS "]")
{
    SECTION("SHA1")
    {
        std::array<u8, SHA1LEN> expected {0xfb, 0x2f, 0x85, 0xc8, 0x85, 0x67, 0xf3, 0xc8, 0xce, 0x9b,
                                          0x79, 0x9c, 0x7c, 0x54, 0x64, 0x2d, 0xc,  0x7b, 0x41, 0xf6};
        CHECK(pwn::crypto::sha1(data) == expected);
    }

    SECTION("SHA256")
    {
        std::array<u8, SHA256LEN> expected {0xe1, 0x2e, 0x11, 0x5a, 0xcf, 0x45, 0x52, 0xb2, 0x56, 0x8b, 0x55,
                                            0xe9, 0x3c, 0xbd, 0x39, 0x39, 0x4c, 0x4e, 0xf8, 0x1c, 0x82, 0x44,
                                            0x7f, 0xaf, 0xc9, 0x97, 0x88, 0x2a, 0x2,  0xd2, 0x36, 0x77};
        CHECK(pwn::crypto::sha256(data) == expected);
    }

    SECTION("SHA512")
    {
        std::array<u8, SHA512LEN> expected {
        ...
        CHECK(pwn::crypto::sha512(data) == expected);
    }
}
