...
#define PWN_UTILS_UPPER_LOWER_CHARSET  CONCAT(PWN_UTILS_LOWER_CHARSET, PWN_UTILS_UPPER_CHARSET)
#define PWN_UTILS_ALNUM_CHARSET CONCAT(PWN_UTILS_UPPER_LOWER_CHARSET, PWN_UTILS_DIGITS_CHARSET)
#define PWN_UTILS_PRINTABLE_CHARSET CONCAT(PWN_UTILS_ALNUM_CHARSET, "!\"#$ % &'()*+,-./:;<=>?@[\\]^_`{|}~ ")
...
namespace pwn::utils
{
    ...
    {
        PWNAPI void seed();
        PWNAPI auto rand() -> u64;
        PWNAPI auto rand(_In_ u32 min, _In_ u32 max) -> u64;
        PWNAPI auto byte() -> u8;
        PWNAPI auto word() -> u16;
        PWNAPI auto dword() -> u32;
        PWNAPI auto qword() -> u64;
        PWNAPI auto string(_In_ u32 length) -> std::wstring;
        PWNAPI auto alnum(_In_ u32 length) -> std::wstring;
        PWNAPI auto buffer(_In_ u32 length) -> std::vector<u8>;
    }
    ...
    PWNAPI auto wstring_to_bytes(_In_ const std::wstring& str) -> std::vector<u8>;
    PWNAPI auto string_to_bytes(_In_ std::string const& str) -> std::vector<u8>;
